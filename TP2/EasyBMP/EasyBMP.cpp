...
void BMIH::display(void) {
  ...
  cout << "biSize: " << (int)biSize << endl
       ...
       << "biClrImportant: " << (int)biClrImportant << endl
       << endl;
}
...
bool BMP::SetColor(int ColorNumber, RGBApixel NewColor) {
  ...
  if (BitDepth != 1 && BitDepth != 4 && BitDepth != 8) {
    if (EasyBMPwarnings) {
      cout
          << "EasyBMP Warning: Attempted to change color table for a BMP object"
          << endl
  ...
  if (!Colors) {
    if (EasyBMPwarnings) {
      cout << "EasyBMP Warning: Attempted to set a color, but the color table"
           << endl
  ...
  if (ColorNumber >= TellNumberOfColors()) {
    if (EasyBMPwarnings) {
      cout << "EasyBMP Warning: Requested color number " << ColorNumber
           << " is outside the allowed" << endl
...
RGBApixel BMP::GetColor(int ColorNumber) {
  ...
  if (BitDepth != 1 && BitDepth != 4 && BitDepth != 8) {
    if (EasyBMPwarnings) {
      cout
          << "EasyBMP Warning: Attempted to access color table for a BMP object"
          << endl
          << "                 that lacks a color table. Ignoring request."
          << endl;
    }
...
bool BMP::SetSize(int NewWidth, int NewHeight) {
  ...
  if (NewWidth <= 0 || NewHeight <= 0) {
    if (EasyBMPwarnings) {
      cout << "EasyBMP Warning: User attempted to set a non-positive width or "
              "height."
           << endl
           << "                 Size remains unchanged at " << Width << " x "
           << Height << "." << endl;
    }
    return false;
  }
...
bool BMP::WriteToFile(const char *FileName) {
  ...
  if (BitDepth != 16) {
    ...
    while (j > -1) {
      ...
      if (!Success) {
        if (EasyBMPwarnings) {
          cout << "EasyBMP Error: Could not write proper amount of data."
               << endl;
        }
        j = -1;
      }
...
bool BMP::ReadFromFile(const char *FileName) {
  ...
  // a safety catch: if any of the header information didn't read properly,
  // abort future idea: check to see if at least most is self-consistent
  ...
  if (bmih.biCompression > 3) {
    if (EasyBMPwarnings) {
      cout << "EasyBMP Error: " << FileName << " is in an unsupported format."
           ...
           << ")" << endl
           << "               The file is probably an old OS2 bitmap or "
              "corrupted."
           << endl;
    }
    SetSize(1, 1);
  ...
  if (BitDepth < 16) {
    ...
    if (NumberOfColorsToRead < TellNumberOfColors()) {
      if (EasyBMPwarnings) {
        cout << "EasyBMP Warning: file " << FileName << " has an underspecified"
             << endl
             << "                 color table. The table will be padded with "
                "extra"
             << endl
  ...
  if (BitDepth != 16) {
    ...
    while (j > -1) {
      ...
      if (BytesRead < BufferSize) {
        j = -1;
        if (EasyBMPwarnings) {
          cout << "EasyBMP Error: Could not read proper amount of data."
               << endl;
        }
      } else {
        bool Success = false;
  ...
  if (!fp) {
    if (EasyBMPwarnings) {
      cout << "EasyBMP Error: Cannot initialize from file " << szFileNameIn
           << "." << endl
           << "               File cannot be opened or does not exist." << endl;
    }
    bmfh.bfType = 0;
    return bmfh;
  }
...
void DisplayBitmapInfo(const char *szFileNameIn) {
  ...
  cout << "File information for file " << szFileNameIn << ":" << endl << endl;
  ...
  cout << "BITMAPINFOHEADER:" << endl
       ...
       << "biClrImportant: " << bmih.biClrImportant << endl
       << endl;
  return;
}
...
void PixelToPixelCopy(BMP &From, int FromX, int FromY, BMP &To, int ToX,
                      int ToY) {
  *To(ToX, ToY) = *From(FromX, FromY);
  return;
}

void PixelToPixelCopyTransparent(BMP &From, int FromX, int FromY, BMP &To,
                                 int ToX, int ToY, RGBApixel &Transparent) {
...
void RangedPixelToPixelCopy(BMP &From, int FromL, int FromR, int FromB,
                            int FromT, BMP &To, int ToX, int ToY) {
  ...
  for (j = FromT; j <= FromB; j++) {
    for (i = FromL; i <= FromR; i++) {
      PixelToPixelCopy(From, i, j, To, ToX + (i - FromL), ToY + (j - FromT));
    }
  }
...
void RangedPixelToPixelCopyTransparent(BMP &From, int FromL, int FromR,
                                       int FromB, int FromT, BMP &To, int ToX,
                                       int ToY, RGBApixel &Transparent) {
  // make sure the conventions are followed
  if (FromB < FromT) {
    int Temp = FromT;
    FromT = FromB;
  ...
  for (j = FromT; j <= FromB; j++) {
    for (i = FromL; i <= FromR; i++) {
      PixelToPixelCopyTransparent(From, i, j, To, ToX + (i - FromL),
                                  ToY + (j - FromT), Transparent);
    }
  }
...
bool EasyBMPcheckDataSize(void) {
  ...
  if (sizeof(ebmpBYTE) != 1) {
    if (EasyBMPwarnings) {
      cout << "EasyBMP Error: ebmpBYTE has the wrong size (" << sizeof(ebmpBYTE)
           << " bytes)," << endl
  ...
  if (sizeof(ebmpWORD) != 2) {
    if (EasyBMPwarnings) {
      cout << "EasyBMP Error: ebmpWORD has the wrong size (" << sizeof(ebmpWORD)
           << " bytes)," << endl
...
bool Rescale(BMP &InputImage, char mode, int NewDimension) {
  ...
  for (int j = 0; j < NewHeight - 1; j++) {
    ...
    InputImage(NewWidth - 1, j)->Red =
        (ebmpBYTE)((1.0 - ThetaJ) * (OldImage(OldWidth - 1, J)->Red) +
                   ThetaJ * (OldImage(OldWidth - 1, J + 1)->Red));
    InputImage(NewWidth - 1, j)->Green =
        (ebmpBYTE)((1.0 - ThetaJ) * (OldImage(OldWidth - 1, J)->Green) +
                   ThetaJ * (OldImage(OldWidth - 1, J + 1)->Green));
    InputImage(NewWidth - 1, j)->Blue =
        (ebmpBYTE)((1.0 - ThetaJ) * (OldImage(OldWidth - 1, J)->Blue) +
  ...
  for (int i = 0; i < NewWidth - 1; i++) {
    ...
    I = (int)floor(ThetaI);
    ThetaI -= I;
    InputImage(i, NewHeight - 1)->Red =
        (ebmpBYTE)((1.0 - ThetaI) * (OldImage(I, OldHeight - 1)->Red) +
                   ThetaI * (OldImage(I, OldHeight - 1)->Red));
    InputImage(i, NewHeight - 1)->Green =
        (ebmpBYTE)((1.0 - ThetaI) * (OldImage(I, OldHeight - 1)->Green) +
                   ThetaI * (OldImage(I, OldHeight - 1)->Green));
    InputImage(i, NewHeight - 1)->Blue =
        (ebmpBYTE)((1.0 - ThetaI) * (OldImage(I, OldHeight - 1)->Blue) +
                   ThetaI * (OldImage(I, OldHeight - 1)->Blue));
  }

  *InputImage(NewWidth - 1, NewHeight - 1) =
      *OldImage(OldWidth - 1, OldHeight - 1);
  return true;
}
